...
def main(rank, world_size, config_path):
    ...
    try:
        ...
        if cell_type_csv and os.path.exists(cell_type_csv):
            ...
            if rank == 0:
                print("Warning: cell_type_csv not provided or not found. Using default cell types.")
            target_cell_types = None  # will use default in dataset class
        
        # Create datasets
        train_dataset = MultiDatasetCellTypeDataset(
            config['data']['base_data_path'], 
            split='train',
            target_cell_types=target_cell_types,  # ← pass loaded list
        ...
        if rank == 0:
            mapping_save_dir = "/home/arism/celltype_results"
            ...
            label_to_idx = train_dataset.cell_type_to_idx  # dict: cell_type → index
            save_label_mapping(label_to_idx, mapping_save_path)
            
            # Also save reverse mapping for convenience (index → cell_type)
        ...
        if rank == 0:
            ...
            save_dir = "/home/arism/celltype_results"
