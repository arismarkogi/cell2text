...
def load_similarity_matrix(pkl_path, expected_size=None):
    """Load precomputed similarity matrix from .pkl"""
    with open(pkl_path, 'rb') as f:
        sim_matrix = pickle.load(f)
    
    if isinstance(sim_matrix, dict) and 'similarity_matrix' in sim_matrix:
        sim_matrix = sim_matrix['similarity_matrix']  # adapt if needed
    
    sim_matrix = np.array(sim_matrix)
    
    if expected_size and sim_matrix.shape[0] != expected_size:
        raise ValueError(f"Similarity matrix size {sim_matrix.shape} doesn't match num_cell_types={expected_size}")
    
    print(f"Loaded similarity matrix of shape {sim_matrix.shape}")
    return sim_matrix

def calculate_ontology_metrics(true_labels, pred_labels, similarity_matrix):
    """Calculate mean semantic similarity between true and predicted cell types."""
    if len(true_labels) == 0:
        return 0.0
    
    similarities = []
    for true_idx, pred_idx in zip(true_labels, pred_labels):
        sim = similarity_matrix[true_idx, pred_idx]
...
def calculate_metrics(results, similarity_matrix=None):
    ...
    # Enhanced ontology-aware metrics
    if similarity_matrix is not None:
        all_similarities = []
        correct_similarities = []
        wrong_similarities = []
        
        for true_idx, pred_idx in zip(labels, pred_classes):
            sim = similarity_matrix[true_idx, pred_idx]
            ...
            if true_idx == pred_idx:
                ...
                wrong_similarities.append(sim)
        
        metrics['ontology'] = {
            'mean_semantic_similarity': np.mean(all_similarities),
            'std_semantic_similarity': np.std(all_similarities),
            'mean_similarity_correct': np.mean(correct_similarities) if correct_similarities else 0.0,
            'std_similarity_correct': np.std(correct_similarities) if len(correct_similarities) > 1 else 0.0,
            'mean_similarity_wrong': np.mean(wrong_similarities) if wrong_similarities else 0.0,
            'std_similarity_wrong': np.std(wrong_similarities) if len(wrong_similarities) > 1 else 0.0,
            'num_correct_predictions': len(correct_similarities),
            'num_wrong_predictions': len(wrong_similarities)
        }
...
def save_results(results, metrics, save_dir, rank, cell_type_names=None):
    ...
    # Add ontology similarity if available
    if 'ontology' in metrics:
        sim_matrix = metrics.get('similarity_matrix')
        if sim_matrix is not None:
            similarities = [
                sim_matrix[int(true), int(pred)]
                for true, pred in zip(results['labels'], results['predictions'])
            ]
            predictions_df['semantic_similarity'] = similarities
...
def print_results(metrics, num_samples, rank):
    ...
    if 'ontology' in metrics:
        print(f"\nONTOLOGY-AWARE METRICS:")
        print("-" * 40)
        ont_metrics = metrics['ontology']
        print(f"  Mean Semantic Similarity: {ont_metrics['mean_semantic_similarity']:.4f} ± {ont_metrics['std_semantic_similarity']:.4f}")
        print(f"  Correct Predictions Similarity: {ont_metrics['mean_similarity_correct']:.4f} ± {ont_metrics['std_similarity_correct']:.4f}")
        print(f"  Wrong Predictions Similarity: {ont_metrics['mean_similarity_wrong']:.4f} ± {ont_metrics['std_similarity_wrong']:.4f}")
        print(f"  Correct/Wrong Count: {ont_metrics['num_correct_predictions']}/{ont_metrics['num_wrong_predictions']}")
...
def main(rank, world_size, config_path, checkpoint_path, output_dir, similarity_pkl=None, label_mapping_json=None):
    ...
    try:
        ...
        if label_mapping_json and rank == 0:
            with open(label_mapping_json, 'r') as f:
                idx_to_label = json.load(f)
                # Convert keys to int
                idx_to_label = {int(k): v for k, v in idx_to_label.items()}
                cell_type_names = [idx_to_label[i] for i in range(len(idx_to_label))]
            print(f"Loaded {len(cell_type_names)} cell type names from {label_mapping_json}")
        ...
        # Load similarity matrix (rank 0 only, but we’ll broadcast if needed)
        similarity_matrix = None
        if similarity_pkl and rank == 0:
            similarity_matrix = load_similarity_matrix(similarity_pkl, test_dataset.num_cell_types)
        
        # For DDP, we can broadcast it to all ranks if needed (optional)
        # But since metric calculation is on rank 0, it’s fine to load only there.
        
        results = evaluate_ddp(model, test_loader, rank, world_size, test_dataset.num_cell_types)
        
        if rank == 0:
            metrics = calculate_metrics(results, similarity_matrix)
            
            # Attach similarity matrix to metrics for saving (optional)
            if similarity_matrix is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--similarity-pkl', default="/home/arism/datasets/cell_type_similarities.pkl",  help='Path to ontology similarity matrix .pkl file')
    ...
    torch.multiprocessing.spawn(
        main,
        args=(args.world_size, args.config, args.checkpoint, args.output_dir, args.similarity_pkl, args.label_mapping),
