...
print("\n--- Preparing for Robust 80/10/10 Split ---")
...
val_target_size = int(total_cells * 0.10)
test_target_size = int(total_cells * 0.10)
...
for donor_id in assignment_pool:
    ...
    if val_fullness <= test_fullness and val_current_size + donor_size < val_target_size * 1.1: # Allow going slightly over
        val_donors.add(donor_id)
        val_current_size += donor_size
    elif test_current_size + donor_size < test_target_size * 1.1:
...
def process_anndata_in_batches(
    sorted_join_ids,
    batch_size,
    data_dir,
    census,
    ...
    """
    Fetches and processes AnnData objects in batches, calculates pathway activities,
    and saves the results.

    Args:
        sorted_join_ids (list): A sorted list of soma_joinids to process.
        batch_size (int): The number of cells to process in each batch.
        ...
        gmt_path (str, optional): Path to the GMT file for gene signatures. 
                                 Defaults to data_dir / 'intermediate_data' / 'h.all.v2025.1.Hs.symbols.gmt'.
        output_dir (str, optional): Directory to save the output h5ad files. 
                                    Defaults to data_dir / 'input_data'.
        chunk_size (int, optional): Chunk size for AUCell calculation. Defaults to 1000.
    """
    if gmt_path is None:
        gmt_path = os.path.join(data_dir, 'intermediate_data', "h.all.v2025.1.Hs.symbols.gmt")
    if output_dir is None:
        output_dir = os.path.join(data_dir, 'input_data')

    os.makedirs(output_dir, exist_ok=True)

    print("--- Starting to fetch and process full AnnData objects in batches ---")
    pbar = tqdm(range(0, len(sorted_join_ids), batch_size), desc="Fetching Batches")
    for i in pbar:
        ...
        pbar.set_description(f"Data fetched from cellxgene; batch {batch_num}")
        ...
        pbar.set_description(f"Minor change to the data; batch {batch_num}")
        ...
        pbar.set_description(f"Count matrix processed; batch {batch_num}")
        ...
        for j in range(n_chunks):
            chunk = ex_matrix.iloc[j*chunk_size:(j+1)*chunk_size]
        ...
        pbar.set_description(f"Signatures gathered; batch {batch_num}")
        ...
        pbar.set_description(f"Pathway 1 done; batch {batch_num}")
        ...
        pbar.set_description(f"Pathway 2 done; batch {batch_num}")

        if batch_data.var.index.name == 'gene_symbol' and 'gene_symbol' in batch_data.var.columns:
            if batch_data.var.index.equals(batch_data.var['gene_symbol']):
                batch_data.var = batch_data.var.drop(columns=['gene_symbol'])
            else:
                batch_data.var = batch_data.var.rename(columns={'gene_symbol': 'gene_symbol_col'})
            batch_data.var.index.name = None

        
        save_dir = os.path.join(output_dir, type+f"{batch_num}")
        os.makedirs(save_dir, exist_ok=True)
        output_path = os.path.join(save_dir, f"batch_{batch_num}.h5ad")

        batch_data.write(output_path)
        pbar.set_description(f"Batch {batch_num} saved")

        del batch_data
        gc.collect()

process_anndata_in_batches(
    train_join_ids,
    args.BS,
    DATA_DIR,
    census,
...
#     pbar.set_description(f"Processing batch {i//batch_size + 1}")
...
#     pbar.set_description(f"data fetched from cellxgene; batch {i//batch_size + 1}")
...
#     pbar.set_description(f"minor change to the data; batch {i//batch_size + 1}")
...
#     pbar.set_description(f"count matrix processed; batch {i//batch_size + 1}")

#     gc.collect()

#     # Load gene signatures
#     GMT_FNAME = DATA_DIR / 'intermediate_data' / "h.all.v2025.1.Hs.symbols.gmt"       ######################## to change location #############################
#     signatures = GeneSignature.from_gmt(str(GMT_FNAME), field_separator="\t")
...
#     # Parameters
#     chunk_size = args.CS  # Adjust based on your memory budget
#     n_chunks = int(np.ceil(ex_matrix.shape[0] / chunk_size))
...
#     for j in range(n_chunks):
#         # print(f"Processing chunk {j+1}/{n_chunks}")
#         chunk = ex_matrix.iloc[j*chunk_size:(j+1)*chunk_size]
...
#     pbar.set_description(f"signatures gathered; batch {i//batch_size + 1}")
...
#     batch_data.obs["pathway1"] = aucs_df.idxmax(axis=1)
#     pbar.set_description(f"pathway 1 done; batch {i//batch_size + 1}")
...
#     pbar.set_description(f"pathway 2 done; batch {i//batch_size + 1}")

#     if batch_data.var.index.name == 'gene_symbol' and 'gene_symbol' in batch_data.var.columns:
#         # Either drop the column, rename it, or remove the index name
#         if batch_data.var.index.equals(batch_data.var['gene_symbol']):
#             # Same content, safe to drop column
#             batch_data.var = batch_data.var.drop(columns=['gene_symbol'])
#         else:
#             # Different content, rename column to avoid conflict
#             batch_data.var = batch_data.var.rename(columns={'gene_symbol': 'gene_symbol_col'})
        
#         # Remove the index name to prevent conflicts
#         batch_data.var.index.name = None

#     # Save results 
#     # batch_data.X = batch_data.raw.X
#     batch_data.write(str(DATA_DIR / 'input_data' / f"batch_{i//batch_size + 1}.h5ad"))  ######################## to change location #############################
#     pbar.set_description(f"batch {i//batch_size + 1} saved")
#     del batch_data
#     gc.collect()
...
gmt_filename = DATA_DIR / 'intermediate_data' / "h.all.v2025.1.Hs.symbols.gmt"
...
for directo in tqdm(in_dirs, desc='final step'):
    if 'train' in directo.split('/')[-1]:
        hf_dataset = process_dataset(input_dir=directo, output_dir=DATA_DIR / 'output_data' / 'train', ontology_filepath=DATA_DIR / "intermediate_data" / "obo.json" ,dataset_name=f"dataset_{i_train}")
        i_train+=1
    elif 'test' in directo.split('/')[-1]:
        hf_dataset = process_dataset(input_dir=directo, output_dir=DATA_DIR / 'output_data' / 'test', ontology_filepath=DATA_DIR / "intermediate_data" / "obo.json" ,dataset_name=f"dataset_{i_test}")
        i_test+=1
    elif 'val' in directo.split('/')[-1]:
        hf_dataset = process_dataset(input_dir=directo, output_dir=DATA_DIR / 'output_data' / 'val', ontology_filepath=DATA_DIR / "intermediate_data" / "obo.json" ,dataset_name=f"dataset_{i_val}")
        i_val+=1
