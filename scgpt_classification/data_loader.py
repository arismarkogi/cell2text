...
class PreprocessedDataset(Dataset):
    """
    A simple PyTorch Dataset wrapper for a Hugging Face Dataset 
    loaded from disk. This is extremely fast and memory-efficient.
    """
    def __init__(self, dataset_dir: Path):
        self.dataset = load_from_disk(str(dataset_dir))
        # Set format to torch for automatic tensor conversion
        self.dataset.set_format(type='torch', columns=['gene_ids', 'values', 'labels'])
    ...
    def __getitem__(self, idx):
        # The dataset.set_format call makes this automatically return tensors
        return self.dataset[idx]

def load_label_mappings(data_dir: Path):
    """Loads the saved label mappings."""
    mapping_file = data_dir / "label_mappings.json"
    if not mapping_file.exists():
        raise FileNotFoundError(f"label_mappings.json not found in {data_dir}")
    ...
    return num_classes, id_to_label, label_to_id

def create_dataloaders(preprocessed_data_dir: Path, config, rank, world_size, logger):
    """
    Creates DataLoaders for train, val, and test splits.
    This now enables TRUE, global shuffling.
    """
    loaders = {}
    ...
    for split in ["train", "val", "test"]:
        ...
        dataset = PreprocessedDataset(split_dir)
        ...
        if world_size > 1:
            sampler = DistributedSampler(
                ...
                rank=rank,
                shuffle=is_train  # Only shuffle for training
            )
        ...
        loader = DataLoader(
            ...
            sampler=sampler,
            shuffle=(is_train and sampler is None), # Shuffle if not DDP
            num_workers=4,
            pin_memory=True
        )
