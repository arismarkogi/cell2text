...
def main(rank, world_size, config_path):
    ...
    try:
        ...
        tissue_csv = config['data'].get('tissue_csv')
        if tissue_csv and os.path.exists(tissue_csv):
            target_tissues = load_tissues_from_csv(tissue_csv)
        else:
            if rank == 0:
                print("Warning: tissue_csv not provided or not found. Using default tissue types.")
            target_tissues = None  # will use default in dataset class
        
        # Create datasets
        train_dataset = MultiDatasetTissueDataset(
            config['data']['base_data_path'], 
            split='train',
            target_tissues=target_tissues,  # ← pass loaded list
        ...
        if rank == 0:
            mapping_save_dir = "/home/arism/tissue_results" # CHNAGE THIS PATH
            ...
            label_to_idx = train_dataset.tissue_to_idx  # dict: tissue → index
            save_label_mapping(label_to_idx, mapping_save_path)
            
            # Also save reverse mapping for convenience (index → tissue)
        ...
        if rank == 0:
            ...
            save_dir = "/home/arism/tissue_results"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"{date_str}_ddp_best_model.pt")
